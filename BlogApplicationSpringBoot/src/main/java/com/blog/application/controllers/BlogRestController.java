...
public class BlogRestController {
    ...
    public ResponseEntity<List<Blog>> getBlogs(HttpServletRequest request) throws UnknownHostException, BlogException {
        long startTime = System.currentTimeMillis();
        logClientHeaders(request);
    ...
    @ApiResponses(value = { @ApiResponse(code = 200, message = "Successfully retrieved the blog"),
                    ...
                    @ApiResponse(code = 500, message = "Internal server error"),
                    @ApiResponse(code = 503, message = "Service Unavailable") })
    public ResponseEntity<Blog> getBlogById(@PathVariable("blogId") int blogId, HttpServletRequest request)
                    throws BlogException {
        logClientHeaders(request);
    ...
    @ApiResponses(value = { @ApiResponse(code = 200, message = "Successfully added the blog"),
                    ...
                    @ApiResponse(code = 500, message = "Internal server error"),
                    @ApiResponse(code = 503, message = "Service Unavailable") })
    public ResponseEntity<String> addBlog(@RequestBody Blog blog, HttpServletRequest request) throws BlogException {
        logClientHeaders(request);
        LOGGER.info(BLOG2, blog);
        // Use toString logging to avoid direct getter call which may not be present in all environments
        LOGGER.info("blog (toString): {}", blog);
    ...
    public ResponseEntity<String> editBlog(@PathVariable("blogId") Long blogId, @RequestBody Blog blog,
                                            HttpServletRequest request) throws BlogException {
        logClientHeaders(request);
        LOGGER.info(BLOG_ID, blogId);
        boolean validation = blogValidator.validateBlog(blog);
    ...
    @ApiResponses(value = { @ApiResponse(code = 200, message = "Successfully deleted the blog post"),
                    ...
                    @ApiResponse(code = 500, message = "Internal server error"),
                    @ApiResponse(code = 503, message = "Service Unavailable") })
    public ResponseEntity<String> deleteBlogPost(@PathVariable("blogId") Long blogId, HttpServletRequest request)
                    throws BlogException {
        logClientHeaders(request);
        LOGGER.info(BLOG_ID, blogId);
    ...
    public ResponseEntity<List<Blog>> getBlogByIdAndUserId(@PathVariable("blogId") int blogPostId,
                                                            @PathVariable("userId") int userId, HttpServletRequest request) throws BlogException {
        logClientHeaders(request);
    ...
    @GetMapping("")
    public ResponseEntity<List<Blog>> getBlogLists(HttpServletRequest request) {
        logClientHeaders(request);
        List<Blog> blogList = blogService.findAll();
    ...
    public ResponseEntity<String> addBlog2(@RequestBody Blog blog, HttpServletRequest request) {
        logClientHeaders(request);
        blogService.addBlog(blog);
    ...
    // Helper to read and log headers
    private void logClientHeaders(HttpServletRequest request) {
        if (request == null) {
            return;
        }
        String clientId = request.getHeader("clientId");
        String uuid = request.getHeader("uuid");
        LOGGER.info("clientId: {}, uuid: {}", clientId, uuid);
    }
